...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  for (int i = 0; i < length; i++) {
    ...
    inputcommand += (char)payload[i];
  }
  Serial.println();

  // Switch on the LED if an 1 was received as first character
  Serial.println(inputcommand);
  //if(0 == strcmp("$LED_ON;", (char*)&payload))
  if(inputcommand == LED_ON)
  {
    digitalWrite(BUILTIN_LED, LOW);  // Turn the LED off by making the voltage HIGH
    Serial.println("Turning LED ON");
  }
  //else if(0 == strcmp("$LED_OFF;", (char*)&payload))
  if(inputcommand == LED_OFF)
  {
    digitalWrite(BUILTIN_LED, HIGH);  // Turn the LED off by making the voltage HIGH
    Serial.println("Turning LED OFF");
  }
  //else if(0 == strcmp("$LED_BLINK;", (char*)&payload))
  if(inputcommand == LED_BLINK)
  {
